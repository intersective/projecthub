...
export default function LearnerRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Step 1: Register user via Better Auth
      ...
      console.log('[Learner] Registration success:', userData);

      // Step 2: Assign learner role
      try {
      ...
      // Redirect to login
      router.push('/learner/login?registered=true');
    } catch (error: any) {
  ...
  const isFormValid = state.fullName.trim() && 
                      state.email.trim() && 
