...
export abstract class AIService {
  ...
  protected async callAI<T>(
    ...
    const { retryable = true, reasoningEffort = 'medium', verbosity = 'medium' } = options;
    const startTime = Date.now();

    const operation = async () => {
      // Build the request parameters with GPT-5 specific options
      ...
      // Add GPT-5 specific parameters if using a GPT-5 model
      if (model.startsWith('gpt-5')) {
        requestParams.reasoning = { effort: reasoningEffort };
        // Merge verbosity into existing text object
        requestParams.text = {
          ...requestParams.text,
          verbosity: verbosity
        };
      ...
      const response = await this.client.responses.parse(requestParams);
